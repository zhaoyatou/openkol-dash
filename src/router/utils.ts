...
/** 判断两个数组彼此是否存在相同值 */
function isOneOfArray(a: Array<string>, b: string[]) {
  const roles = b.map(i => i);
  return Array.isArray(a) && Array.isArray(roles)
    ? intersection(a, roles).length > 0
      ? true
      : false
    : true;
}
...
function filterNoPermissionTree(data: RouteComponent[]) {
  const currentRoles =
    storageLocal().getItem<DataInfo>(sessionKey)?.roles ?? [];

  const newTree = cloneDeep(data).filter((v: any) =>
    isOneOfArray(
      v.meta?.roles,
      currentRoles.map(r => r.name)
    )
  );
  newTree.forEach(
  ...
  if (getConfig()?.CachingAsyncRoutes) {
    ...
    if (asyncRouteList && asyncRouteList?.length > 0) {
      ...
      return new Promise(resolve => {
        ...
        storageLocal().setItem(key, asyncRoutes);
        resolve(router);
      });
    }
  } else {
